...
class PurchaseView extends React.Component {
	...
	componentDidMount(){
		this.cancelControl = makeCancelable(
			Fetch.get("/purchase/user/" + this.state.purchase.id + '/', {is_done: this.state.purchase.is_done}),
			(value) => {
				const data = value.body;
				if(this.state.is_done){
					this.setState({ purchase: data.purchase, fuel_type: data.fuel_type, loading: false });
				}else{
		...
		return (
			<ScrollView style={tailwind('mb-4 px-4')}>
				...
				<Text style={[tailwind('text-xs')]}>{
					...
					<View>
						<Text><Text style={typefaces.pb}>Fecha:
							</Text> {formatISODate(this.state.purchase.created_at, 'dd/MM/yyyy HH:mm')}</Text>
						<Text><Text style={typefaces.pb}>Expira:
							</Text> {formatISODate(this.state.purchase.code_expiry_date, 'dd/MM/yyyy HH:mm')}</Text>
						<Text><Text style={typefaces.pb}>Cliente: </Text>{this.fullName()}</Text>
						<Text>
							<Text style={typefaces.pb}>Vehículo: 
							</Text> {this.state.purchase.vehicle.number} 
								{this.state.purchase.vehicle.alias ? '(' + this.state.purchase.vehicle.alias + ')' : ''}
						</Text>
					</View>
					<View style={tailwind('mt-3')}>
						<Text>
							<Text style={typefaces.pb}>Estación: </Text>{this.gasStation().name}
						</Text>
						<Text>
							<Text style={typefaces.pb}>Dirección: </Text>{this.gasStation().address}
						</Text>
					</View>
					{
						this.state.is_done ?
						<View style={tailwind('mt-3')}>
							<Text>
								<Text style={typefaces.pb}>Gasolina: </Text>{this.state.fuel_type.name}
							</Text>
							<Text>
								<Text style={typefaces.pb}>Galones:
								</Text> {parseFloat(this.state.purchase.gallons).toFixed(2)}
							</Text>
						</View> : <View></View>
					}
					<View style={tailwind('flex justify-center items-center mt-4')}>
						...
						this.state.generating ?
							<View style={[tailwind('flex flex-row justify-center')]}>
								<ActivityIndicator animating color="black" size="large" />
							</View> : this.state.in_process ?
							<View>
								<Text style={[tailwind('text-xl text-gray-800 text-center'), typefaces.pm]}>
		...
		>
			<View style={tailwind('w-full bg-white rounded-lg')}>
				<View style={tailwind('p-6 rounded-md')}>
					...
					<CheckRoundedIcon />
						<Text style={[tailwind('text-sm ml-4'), typefaces.psb]}>¿Está seguro que desea cancelar la compra?</Text>
					</View>
